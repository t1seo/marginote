...
**Candidate verification is in progress. Version 0.1.2 has not been published yet.** This record separates code changes, executed tests, signed release provenance, branch previews, and the live Community result.
...
| `text-decoration` compatibility, two locations | Use a dotted bottom border and suppress duplicate native underlines | Eight actual 1.13.7 mode/theme/hover comparisons and four wrapped-selection checks pass; repeat legacy-host QA and live preview pending |
...
Independent delivery review, publication, downloaded-asset QA, and the live Community rescan remain in progress. A preview scan was also requested for the exact public candidate commit before publication.
